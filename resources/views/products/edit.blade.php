...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-3xl font-bold tracking-tight text-black">Edit Product</h1>
      <p class="mt-1 text-sm text-brand-gray">Update product details and stock</p>
    </div>
    ...
    <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" class="p-6 space-y-6">
      ...
      <div>
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1">
            ...
            <p class="mt-1 text-xs text-gray-500">PNG, JPG, WEBP up to 2MB. Leave empty to keep current image.</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input name="name" value="{{ old('name', $product->name) }}" placeholder="e.g., iPhone 15 Pro" class="w-full rounded-lg border-gray-300 focus:border-brand-red focus:ring-brand-red" required>
          @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>

        <div>
          <label class="block text-sm font-medium text-black mb-2">Price (€) *</label>
          <input name="price" value="{{ old('price', $product->price) }}" placeholder="0.00" type="number" step="0.01" min="0" class="w-full rounded-lg border-gray-300 focus:border-brand-red focus:ring-brand-red" required>
          @error('price')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>

        <div>
          <label class="block text-sm font-medium text-black mb-2">Category</label>
          <input name="category" value="{{ old('category', $product->category) }}" placeholder="Electronics" list="categories" class="w-full rounded-lg border-gray-300 focus:border-brand-red focus:ring-brand-red">
        ...
        <div>
          ...
          <input name="stock" value="{{ old('stock', $product->stock) }}" placeholder="0" type="number" min="0" class="w-full rounded-lg border-gray-300 focus:border-brand-red focus:ring-brand-red">
      ...
      <div>
        ...
        <textarea name="description" rows="4" placeholder="Brief product description" class="w-full rounded-lg border-gray-300 focus:border-brand-red focus:ring-brand-red">{{ old('description', $product->description) }}</textarea>
